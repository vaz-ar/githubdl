...
def set_logging(log_level: int) -> None:
    log_level = set_default_if_empty(log_level, logging.INFO)
    logging.basicConfig(
        stream=sys.stdout, level=log_level, format='%(asctime)s - %(name)-12s - %(levelname)-8s - %(message)s'
    )


def save_file_to_path(
    repo_url: str, download_filename: str, target_path: str, github_token: str, reference: str
) -> None:
...
def dl_info(repo_url: str, github_token: str, log_level: str, info_type: str) -> str:
...
def dl_tags(repo_url: str, github_token: str = '', log_level: str = '') -> str:
    return dl_info(repo_url, github_token, log_level, 'tags')


def dl_branches(repo_url: str, github_token: str = '', log_level: str = '') -> str:
